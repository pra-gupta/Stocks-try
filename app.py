...
def load_stock_master():
    try:
        ...
        column_mapping = {
            ...
            "company": "Company", "Company Name": "Company", "COMPANY": "Company",
            "sector": "Sector", "Industry": "Sector", "SECTOR": "Sector"
        }
        df = df.rename(columns=column_mapping)
        return df[["Symbol", "Company", "Sector"]].dropna()
    except FileNotFoundError:
        st.warning("`master_stock_list.csv` not found. Falling back to public Nifty 500 list.")
        url = "https://raw.githubusercontent.com/kprohith/nse-stock-analysis/master/ind_nifty500list.csv"
        df = pd.read_csv(url)
        df = df.rename(columns={"Symbol": "Symbol", "Company Name": "Company", "Industry": "Sector"})
        return df[["Symbol", "Company", "Sector"]].dropna()
...
# 3. Live scan worker for selected sector only
@st.cache_data(ttl=3600)
def fetch_sector_live_data(sector_name):
    sector_df = df_master[df_master['Sector'] == sector_name]
    results = []
    financial_histories = {}
    ...
    for i, (_, row) in enumerate(sector_df.iterrows()):
        ...
        try:
            ...
            # Repositioned: Profit Breakout YoY (%) placed directly adjacent to Price (₹)
            results.append({
                "Ticker": symbol,
...
if st.button("Run Live Sector Scan"):
    df, financial_histories = fetch_sector_live_data(selected_sector)
    ...
    if not df.empty:
        ...
        st.error("No data fetched. Check your master CSV ticker symbols or try again later.")
